...
public class Beginner{
	...
	public Beginner(String idpd){
			...
			gbc.gridwidth=1;
			panel.add(mal_p,gbc);
			JLabel tabla= new JLabel("|Table012");
			...
			area.setBorder(BorderFactory.createMatteBorder( 6, 0, 0, 0,new Color(255,215,0)));
			area.setBorder(BorderFactory.createCompoundBorder(area.getBorder(), BorderFactory.createEmptyBorder(0,47 , 0, 0)));
	...
	private class ClassInter implements ActionListener,KeyListener{
	  ...
	  public void keyTyped(KeyEvent event) {
		  	char validar = event.getKeyChar();
			if(!(Character.isDigit(validar) || (validar==event.VK_BACK_SPACE) || validar==event.VK_DELETE)){
				event.consume();
				JOptionPane.showMessageDialog(null,"Introduce  a number of 5 digits");
		}
	 }
...
//método que muestra tiradas de la partida seleccionada
	public String  mostrarTiradas(){
			ArrayList<String> list =bases.leerTiradasBeginner(idColumna);
			String cad="";
			for(String lis:list){
				cad+=lis+"\n";
